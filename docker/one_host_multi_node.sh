...
docker run -d -p 40001:4000 -p 50001:5000 --net it-chain-default-network -v it-chain-default-volume:/go/src/github.com/DE-labtory/engine/.tmp \
-v /var/run/docker.sock:/var/run/docker.sock --ip 172.88.1.2 \
-v $abspath/docker_multi_config_leader.yaml:/go/src/github.com/DE-labtory/engine/conf/config.yaml teamit/itchain:latest > /dev/null 2>&1
...
do
	...
	-v it-chain-default-volume:/go/src/github.com/DE-labtory/engine/.tmp \
	-v /var/run/docker.sock:/var/run/docker.sock --ip 172.88.$i.2 \
	-v $abspath/docker_multi_config_member_$i.yaml:/go/src/github.com/DE-labtory/engine/conf/config.yaml teamit/itchain:latest \
	> /dev/null 2>&1
done
